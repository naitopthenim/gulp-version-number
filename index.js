...
function version(v) {
  ...
  switch (v) {
    ...
    case "%DT%":
      v =
        DT.getFullYear() +
        leadZero(DT.getMonth() + 1, 2) +
        leadZero(DT.getDate(), 2) +
        leadZero(DT.getHours(), 2) +
        leadZero(DT.getMinutes(), 2) +
        leadZero(DT.getSeconds(), 2);
      break;
    case "%TS%":
    ...
    if (config.length) {
      for (var i = 0, len = config.length; i < len; i++) {
        ...
        if (Array.isArray(config[i])) {
          ...
          if (v === null) v = versionNumberList.main;
        } else {
          rep = config[i];
    ...
    if (config["to"]) {
      ...
      if (Array.isArray(apList)) {
        ...
        for (var type in apRule) {
          !versionNumberList[type] &&
            (versionNumberList[type] = apRule[type]["value"] ? version(apRule[type]["value"]) : versionNumberList.main);
